...
class SetoranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Kirim notifikasi ke musyirafah
        $musyirafahList = User::where('role', 'musyirafah')->get();
        if ($musyirafahList->isNotEmpty()) {
            Notification::send($musyirafahList, new SetoranSubmitted($setoran));
        }

        // Kirim email ke orang tua (parent_email)
        $santri = User::find($request->user_id);
        if ($santri && !empty($santri->parent_email)) {
            Notification::route('mail', $santri->parent_email)
                ->notify(new SetoranSubmitted($setoran));
        }

        return redirect()->route('setoran.index')->with('success', 'Setoran berhasil disimpan dan notifikasi telah dikirim.');
    }
